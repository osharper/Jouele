(function($) {
  ...
  var methods = {
    init: function(el) {
      var thisSelector = methods._render(el);

      filename = 'jquery.jouele.js'
      ...
      if (swfPathA = $(el).attr ('data-swfSource')) swfPath = swfPathA
      ...
      $ (thisSelector).find ('.jouele-invisible-object').jPlayer ({
        ...
        ready: function (event) {
          ...
          $ (thisSelector).find ('.jouele-mine').mousedown (function (e) {
            ...
            e.stopPropagation ()
            e.preventDefault ()
            return methods._willSeekTo (thisSelector, me, e.pageX - $ (thisSelector).find ('.jouele-mine').offset ().left)
          })
        ...
        progress: function (event) {
          ...
          methods._updateLoadBar (thisSelector, event.jPlayer.status.seekPercent)
        ...
        timeupdate: function (event) {
          ...
          if (
            ...
            methods._updatePlayBar (thisSelector, playpx)
            methods._updateTimeDisplay (thisSelector, curtime)
    ...
    _updateLoadBar: function (playerSelector, seekPercent) {
    ...
    _updatePlayBar: function (playerSelector, pixels) {

      $ (playerSelector).find ('.jouele-play-lift').css ('left', pixels  + 'px')//- this.options.borderRadius
      $ (playerSelector).find ('.jouele-play-bar').css ('width', pixels + widgetOptions.borderRadius + 'px')

    },

    _willSeekTo: function (playerSelector, playerObject, tryPixels) {

      //tryPixels += (this.options.borderRadius);

      var maxWidth = $ (playerSelector).find ('.jouele-progress-area').width ()
      var minWidth = widgetOptions.borderRadius*2
      var loadWidth = $ (playerSelector).find ('.jouele-load-bar').width ()
      var pixels = Math.min (Math.max (tryPixels, widgetOptions.borderRadius), loadWidth - widgetOptions.borderRadius)
      var playhead = (pixels-widgetOptions.borderRadius)/(maxWidth-minWidth)
      var playheadSeekable = (pixels-widgetOptions.borderRadius)/(loadWidth-minWidth)

      if ((maxWidth == 0) || (loadWidth == 0)) playheadSeekable = playhead = 0

      $ (playerSelector).find ('.jouele-buffering').stop ().fadeTo (1, 1)

      this._updatePlayBar (playerSelector, pixels)
      this._updateTimeDisplay (playerSelector, $ (playerSelector).data ('totalTime') * playhead)
  ...
  $.fn.jouele = function(options) {
      ...
      widgetOptions = $.extend({}, defaultOptions, options);
